...
public static class DataExtensions
{
    ...
    public static async Task SeedGamesFromRawgAsync(this WebApplication app)
    {
        ...
        {
            var rawgGenreName = rawgGame.Genres?.FirstOrDefault()?.Name ?? "Other";

            if (!genreCache.TryGetValue(rawgGenreName, out var genre))
            {
                genre = await context.Genres.FirstOrDefaultAsync(g => g.Name == rawgGenreName);
                if (genre is null)
                {
                ...
                genreCache[rawgGenreName] = genre;
            }
        ...
        await context.SaveChangesAsync();
        Console.WriteLine($"[Seed] Seeded {allGames.Count} games and {genreCache.Count} genres.");
